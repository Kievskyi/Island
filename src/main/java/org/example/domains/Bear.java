...
public class Bear extends Predator {
    ...
    private String icon = AnimalIcons.BEAR.getIcon();
    ...
    public void eat(Animal animal, int length, int width) {
        ...
        if (animal.getClass().getSuperclass() == Predator.class) {
                        ...
                        if (chances.getValue() > 0) {
                            ...
                            if (random <= chances.getValue()) {
                                if (satiety < max_satiety) {        //Here we 100% will kill prey animal
                                    double hunger = max_satiety - satiety;
                                    double weightOfPrey = animal.getWeight() * animal.getMax_amount_in_cell();
                                    if (weightOfPrey <= hunger) {
                                        satiety += weightOfPrey;
                                    } else {
                                        satiety += hunger;
                                    }
                                }
        ...
        } else if (animal.getClass().getSuperclass() == Herbivore.class) {
            for (Map.Entry<String, Integer> chances : chances_to_kill.entrySet()) {
                if (chances.getKey().equals(animal.getKind_of_animal())) {
                    if (chances.getValue() > 0) {
                        int random = ThreadLocalRandom.current().nextInt(0, 101);
